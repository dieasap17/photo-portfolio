...
### 阶段 8.1：Bug 修复 ✅
- [x] **头像拉伸修复**：原图 1086×1448 被 `scale=280:280` 压成正方形，改用 `scale=280:-1` 保持 3:4 比例，CSS cover 自动裁切
- [x] **进度条拖动修复**：倍速按钮从 `bottom: 12px; left: 12px` 移至 `bottom: 56px; right: 16px`，不再覆盖原生进度条
- [x] **倍速按钮归位**：集成到右下角（靠近音量键区域），避开进度条

### 阶段 9：项目文档完善 ✅
- [x] **项目手册**：`docs/project-handbook.md` — 涵盖项目速览、架构、文件地图、内容管理（增删改）、视频规范、部署说明、缓存策略、备份策略、本地开发、下线流程、常见问题、技术债务
- [x] 手册面向未来维护者（人类或 AI），清晰直观可操作

### 备份清理 ✅
- [x] 删除全部 4 个本地备份目录（_backup、_backup-compressed-v1、_backup-portrait、_backup-product）
- [x] 共释放 **3,327 MB**（3.3 GB）磁盘空间
- [x] 源文件已在网盘保留，无需本地冗余
